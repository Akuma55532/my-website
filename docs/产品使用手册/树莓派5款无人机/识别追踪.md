...
实际飞行，我们开启脚本的开机自启动程序，使用遥控器控制起飞后再切换成机载电脑控制模式。

1. 检查开启开机自启动程序，将准备测试的脚本重命名为`detect.py`
2. 给无人机插上遥控器接收机，放到测试场地的起飞点，上电等待。
3. 确保遥控器的`ch7`档位是遥控器控制模式，等到无人机程序启动（飞控成功连接树莓派，程序启动时蜂鸣器会有提示音），解锁，起飞。
4. 飞到目标高度后，将遥控器`ch7`挡位切换为机载电脑控制模式，测试无人机是否跟随。